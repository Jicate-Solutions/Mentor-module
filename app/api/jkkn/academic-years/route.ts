...
/**
 * Transform MyJKKN API academic year response to match our interface
 */
function transformAcademicYearData(apiData: any) {
  return {
    id: apiData.id,
    name: apiData.name || apiData.academic_year_name || 'Unnamed Academic Year',
...
/**
 * GET /api/jkkn/academic-years
 * Fetch academic years from JKKN API (server-side with secure API key)
 *
 * Query params:
 ...
 * - academic_year_name: string (optional)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const academicYearName = searchParams.get('academic_year_name');
    ...
    if (academicYearName) {
      queryParams.append('academic_year_name', academicYearName);
    }

    // Call JKKN API
    const url = `${baseUrl}/api-management/academic/academic-years?${queryParams.toString()}`;

    console.log('[Academic Years API] Fetching:', url);
    ...
    if (!response.ok) {
      ...
      console.error('[Academic Years API] Error:', response.status, errorData);
    ...
    console.log('[Academic Years API] Response:', {
      count: data.count,
    ...
    let transformedData = {
      ...data,
      data: data.data ? data.data.map(transformAcademicYearData) : []
    };
    ...
    if (institutionFilter !== null) {
      ...
      console.log(`[Academic Years] Filtered for institution ${institutionFilter}: ${transformedData.data.length} results`);
    }
