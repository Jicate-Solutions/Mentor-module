...
/**
 * GET /api/jkkn/regulations
 * Fetch regulations from JKKN API (server-side with secure API key)
 *
 * Query params:
 ...
 * - regulation_year: number (optional)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const regulationYear = searchParams.get('regulation_year');
    ...
    if (regulationYear) {
      queryParams.append('regulation_year', regulationYear);
    }

    // Call JKKN API
    const url = `${baseUrl}/api-management/academic/regulations?${queryParams.toString()}`;

    console.log('[Regulations API] Fetching:', url);
    ...
    let transformedData = {
      ...data,
      data: data.data ? data.data.map(transformRegulationData) : []
    };
    ...
    if (institutionFilter !== null) {
      ...
      console.log(`[Regulations] Filtered for institution ${institutionFilter}: ${transformedData.data.length} results`);
    }
